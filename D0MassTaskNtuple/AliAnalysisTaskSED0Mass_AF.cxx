...
#include "AliAnalysisTaskSED0Mass_AF.h"
...
/// \cond CLASSIMP
ClassImp(AliAnalysisTaskSED0Mass_AF);
/// \endcond

//________________________________________________________________________
AliAnalysisTaskSED0Mass_AF::AliAnalysisTaskSED0Mass_AF():
  AliAnalysisTaskSE(),
  fOutputMass(0),
  fOutputMassPt(0),
  fOutputMassY(0),
...
//________________________________________________________________________
AliAnalysisTaskSED0Mass_AF::AliAnalysisTaskSED0Mass_AF(const char *name,AliRDHFCutsD0toKpi* cuts):
...
fCandidateNtuple(0x0)
{
  ...
  DefineOutput(9, TList::Class()); //My private output
    // Output slot #10 stores the nTuple
    DefineOutput(10, TNtuple::Class());
}
...
AliAnalysisTaskSED0Mass_AF::~AliAnalysisTaskSED0Mass_AF()
{
  ...
  if (fhMultVZEROTPCoutTrackCorr) {
    ...
    if(fCandidateNtuple){
        delete fCandidateNtuple;
        fCandidateNtuple = 0;
    }
}  

//________________________________________________________________________
void AliAnalysisTaskSED0Mass_AF::Init()
{
  /// Initialization
...
//________________________________________________________________________
void AliAnalysisTaskSED0Mass_AF::UserCreateOutputObjects()
{
  ...
  fDistr->Add(fhMultVZEROTPCoutTrackCorr);
    ...
    fCandidateNtuple = new TNtuple(nameoutput,"Candidates variables ntuple","D0mass:D0barmass:pt:eta:phi:dca:costsD0:costsD0bar:d0xd0:costp:costpxy:L:Lxy:nL:nLxy:pt1:pt2:eta1:eta2:phi1:phi2:d01:d02:TOFsig1:TOFsig2:TPCsig1:TPCsig2:flagSB:issel");
  ...
  PostData(9,fOutputMassY);
    PostData(10,fCandidateNtuple);

  return;
}

//________________________________________________________________________
void AliAnalysisTaskSED0Mass_AF::UserExec(Option_t */*option*/)

{
  ...
  if(!inputArray || !aod) {
    printf("AliAnalysisTaskSED0Mass_AF::UserExec: input branch not found!\n");
    return;
  }
  ...
  if(fReadMC) {
    ...
    if(!mcArray) {
      printf("AliAnalysisTaskSED0Mass_AF::UserExec: MC particles branch not found!\n");
      return;
    }
    ...
    if(!mcHeader) {
      printf("AliAnalysisTaskSED0Mass_AF::UserExec: MC header branch not found!\n");
      return;
    }
  }
  ...
  for (Int_t iD0toKpi = 0; iD0toKpi < nInD0toKpi; iD0toKpi++) {
	...
	DrawDetSignal(d, fDetSignal);
        ...
        if(fFillNtuple){
     FillCandidateNtuple(d,mcArray,fCuts,aod);
        }
...
void AliAnalysisTaskSED0Mass_AF::DrawDetSignal(AliAODRecoDecayHF2Prong *part, TList *ListDetSignal)
{
  //
...
//____________________________________________________________________________
void AliAnalysisTaskSED0Mass_AF::FillVarHists(AliAODEvent* aod,AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliRDHFCutsD0toKpi *cuts, TList *listout){
  //
...
//____________________________________________________________________________
void AliAnalysisTaskSED0Mass_AF::FillMassHists(AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliAODMCHeader *mcHeader, AliRDHFCutsD0toKpi* cuts, TList *listout){
  //
...
AliAODVertex* AliAnalysisTaskSED0Mass_AF::GetPrimaryVtxSkipped(AliAODEvent *aodev){
...
//________________________________________________________________________
void AliAnalysisTaskSED0Mass_AF::Terminate(Option_t */*option*/)
{
  /// Terminate analysis
  //
...
//________________________________________________________________________
void AliAnalysisTaskSED0Mass_AF::CreateImpactParameterHistos(){
...
Float_t AliAnalysisTaskSED0Mass_AF::GetTrueImpactParameter(AliAODMCHeader *mcHeader, TClonesArray* arrayMC, AliAODMCParticle *partD0) const {
  /// true impact parameter calculation

  printf(" AliAnalysisTaskSED0Mass_AFV1::GetTrueImpactParameter() \n");
...
//_________________________________________________________________________________________________
Int_t AliAnalysisTaskSED0Mass_AF::CheckOrigin(TClonesArray* arrayMC, AliAODMCParticle *mcPartCandidate) const {
  //  obsolete method
  /// checking whether the mother of the particles come from a charm or a bottom quark
  //
  printf(" AliAnalysisTaskSED0Mass_AF V1::CheckOrigin() \n");
...
void AliAnalysisTaskSED0Mass_AF::CreateMCAcceptanceHistos(){
...
void AliAnalysisTaskSED0Mass_AF::FillMCAcceptanceHistos(TClonesArray *arrayMC, AliAODMCHeader *mcHeader){
...
Bool_t AliAnalysisTaskSED0Mass_AF::CheckAcc(TClonesArray* arrayMC,Int_t nProng, Int_t *labDau){
...
void AliAnalysisTaskSED0Mass_AF::NormIPvar(AliAODEvent *aod, AliAODRecoDecayHF2Prong *part,TClonesArray *arrMC){
  Double_t normIP[2];
  Double_t pointD0[9];
...
//________________________________________
void AliAnalysisTaskSED0Mass_AF::FillCandidateNtuple(AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliRDHFCutsD0toKpi *cuts, AliAODEvent *evt){
    
    
    //Printf("AliAnalysisTaskSED0Mass_AF::FillCandidateNtuple");
    Int_t pdgDgD0toKpi[2]={321,211};
    Int_t lab=-9999;
    if(fReadMC) lab=part->MatchToMC(421,arrMC,2,pdgDgD0toKpi); //return MC particle label if the array corresponds to a D0, -1 if not
    
    Int_t isSelectedPID=3;
    if(!fReadMC || (fReadMC && fUsePid4Distr)) isSelectedPID=cuts->IsSelectedPID(part); //0 rejected,1 D0,2 Dobar, 3 both
    
    Float_t minvD0 = 99.;
    Float_t minvD0bar = 99.;
    Float_t ptD  = 99.;
    Float_t etaD = 99.;
    Float_t phiD = 99.;
    Float_t dcaD = 99.;
    Float_t cosThetaStarD0 = 99.;
    Float_t cosThetaStarD0bar = 99.;
    Float_t d0timesd0=99.;
    Float_t cosPointingAngle = 99.;
    Float_t cosPointingAngleXY = 99.;
    Float_t decayLength = -1;
    Float_t decayLengthxy=-1;
    Float_t normalizedDecayLength = -1;
    Float_t normalizedDecayLengthxy=-1;
    Float_t ptProng[2]={-99,-99};
    Float_t etaProng[2]={-99,-99};
    Float_t phiProng[2]={-99,-99};
    Float_t d0Prong[2]={-99,-99};
    Float_t TOFsigProng[2]={-99,-99};
    Float_t TPCsigProng[2]={-99,-99};
    Float_t flagSB=-2;
    Float_t isseltopocuts=fCuts->IsSelected(part,AliRDHFCuts::kAll,evt);;
    
    Float_t ntuplevar[29];
    /*
    TObjArray daughterTracksArray;
    daughterTracksArray.AddAt((AliAODTrack*)part->GetDaughter(0), 0);
    daughterTracksArray.AddAt((AliAODTrack*)part->GetDaughter(1), 1);
    AliAODTrack *prong0=(AliAODTrack*)daughterTracksArray.UncheckedAt(0);
    AliAODTrack *prong1=(AliAODTrack*)daughterTracksArray.UncheckedAt(1);
     */
    ...
    minvD0 = part->InvMassD0();
    minvD0bar = part->InvMassD0bar();
    ptD = part->Pt();
    etaD = part->Eta();
    phiD = part->Phi();
    dcaD = part->GetDCA();
    ...
    cosThetaStarD0bar = part->CosThetaStarD0bar();
    cosPointingAngle = part->CosPointingAngle();
    cosPointingAngleXY = part->CosPointingAngleXY();
    decayLength = part->DecayLength2();
    decayLengthxy = part->DecayLengthXY();
    normalizedDecayLength = part->NormalizedDecayLength2();
    normalizedDecayLengthxy = decayLengthxy/part->DecayLengthXYError();
    
    
    ptProng[0]=prong0->Pt();
    ptProng[1]=prong1->Pt();
    etaProng[0]=prong0->Eta();
    etaProng[1]=prong1->Eta();
    phiProng[0]=prong0->Phi();
    phiProng[1]=prong1->Phi();
    d0Prong[0]=part->Getd0Prong(0);
    d0Prong[1]=part->Getd0Prong(1);
    
    d0timesd0=d0Prong[0]*d0Prong[1];
    
    //AliESDtrack *esdtrack1 = new AliESDtrack((AliVTrack*)daughterTracksArray.UncheckedAt(0));
    //AliESDtrack *esdtrack2 = new AliESDtrack((AliVTrack*)daughterTracksArray.UncheckedAt(1));
    AliESDtrack *esdtrack1 = new AliESDtrack((AliVTrack*)prong0);
    AliESDtrack *esdtrack2 = new AliESDtrack((AliVTrack*)prong1);
    
    TOFsigProng[0]=esdtrack1->GetTOFsignal();
    TOFsigProng[1]=esdtrack2->GetTOFsignal();
    TPCsigProng[0]=esdtrack1->GetTPCsignal();
    TPCsigProng[1]=esdtrack2->GetTPCsignal();
    
    
    if(fReadMC){
        //if (((AliAODMCParticle*)arrMC->At(lab))->GetPdgCode() == 421) flagSB=1; //D0
        //else if (((AliAODMCParticle*)arrMC->At(lab))->GetPdgCode() == -421) flagSB=2; //D0bar
        //else flagSB=0; //background
        ...
        else if ((TMath::Abs(pdgProng[0]) == 321 && TMath::Abs(pdgProng[1]) == 211)){
        //ptk=prong0->Pt();
        //ptpi=prong1->Pt();
        flagSB=2;
        }
        else flagSB=0;
        //Printf("MC flagSB=%f",flagSB);
    }//end fReadMC
    else {
        if(isSelectedPID==1) flagSB=1;
        else if(isSelectedPID==2) flagSB=2;
        else if(isSelectedPID==3) flagSB=3;
        else flagSB=0; //background
        //Printf("Data flagSB=%f",flagSB);
    }

    //fCandidateNtuple = new TNtuple(nameoutput,"Candidates variables ntuple","D0mass:D0barmass:pt:eta:phi:dca:costsD0:costsD0bar:d0xd0:costp:cotpxy:L:Lxy:nL:nLxy:ptk:ptpi:etak:etapi:phik:phipi:d0k:d0pi:TOFsigk:TOFsigpi:TPCsigk:TPCsigpi:flagSB");
    
    ntuplevar[0]=minvD0;
    ntuplevar[1]= minvD0bar;
    ntuplevar[2]= ptD;
    ntuplevar[3]= etaD;
    ntuplevar[4]= phiD;
    ntuplevar[5]= dcaD;
    ntuplevar[6]= cosThetaStarD0;
    ntuplevar[7]= cosThetaStarD0bar;
    ntuplevar[8]= d0timesd0;
    ntuplevar[9]= cosPointingAngle;
    ntuplevar[10]= cosPointingAngleXY;
    ntuplevar[11]= decayLength;
    ntuplevar[12]= decayLengthxy;
    ntuplevar[13]= normalizedDecayLength;
    ntuplevar[14]= normalizedDecayLengthxy;
    ntuplevar[15]= ptProng[0];
    ntuplevar[16]= ptProng[1];
    ntuplevar[17]= etaProng[0];
    ntuplevar[18]= etaProng[1];
    ntuplevar[19]= phiProng[0];
    ntuplevar[20]= phiProng[1];
    ntuplevar[21]= d0Prong[0];
    ntuplevar[22]= d0Prong[1];
    ntuplevar[23]= TOFsigProng[0];
    ntuplevar[24]= TOFsigProng[1];
    ntuplevar[25]= TPCsigProng[0];
    ntuplevar[26]= TPCsigProng[1];
    ntuplevar[27]= flagSB;
    ntuplevar[28]= isseltopocuts;
    
    
    

    //Printf("AliAnalysisTaskSED0Mass_AF::FillCandidateNtuple Filling Ntuple");
    //fCandidateNtuple->Fill(minvD0,minvD0bar,ptD,etaD,phiD,dcaD,cosThetaStarD0,cosThetaStarD0bar,d0timesd0,cosPointingAngle,cosPointingAngleXY,decayLength,decayLengthxy,normalizedDecayLength,normalizedDecayLengthxy,ptProng[0],ptProng[1],etaProng[0],etaProng[1],phiProng[0],phiProng[1],d0Prong[0],d0Prong[1],TOFsigProng[0],TOFsigProng[1],TPCsigProng[0],TPCsigProng[1],flagSB);
    fCandidateNtuple->Fill(ntuplevar);
    
    //daughterTracksArray.Clear();
}

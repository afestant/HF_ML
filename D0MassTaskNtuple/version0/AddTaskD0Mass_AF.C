AliAnalysisTaskSED0Mass_AF *AddTaskD0Mass_AF(Int_t flag=0/*0 = D0,1 = LS*/,Bool_t readMC=kFALSE,
				       ...
				       Bool_t DrawDetSignal=false, Bool_t PIDCheck=false, Bool_t FillMassY=false, Bool_t FillMCAcc=true,Int_t AODProtection = 1, Bool_t fillNtuple=kFALSE)
{
  //
  ...
  if(flag==0){
   ...
   //cuts object
    ...
    if(flagD0D0bar==2)out9name+="D0bar";
      ...
      out10name ="coutputCandidateNtuple";
  ...
  AliAnalysisTaskSED0Mass_AF *massD0Task = new AliAnalysisTaskSED0Mass_AF(taskname.Data(),RDHFD0toKpi);
  ...
  massD0Task->SetDoMCAcceptanceHistos(FillMCAcc);
    massD0Task->SetFillNtuple(fillNtuple);
  ...
  AliAnalysisDataContainer *coutputmassD09 = mgr->CreateContainer(out9name,TList::Class(),AliAnalysisManager::kOutputContainer, filename.Data()); //mass vs y
    AliAnalysisDataContainer *coutputmassD010 = mgr->CreateContainer(out10name,TNtuple::Class(),AliAnalysisManager::kOutputContainer, filename.Data()); //mass vs pt vs impt par

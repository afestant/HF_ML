...
class AliAnalysisTaskSED0Mass : public AliAnalysisTaskSE
{
 ...
 private:
  ...
  void     FillMassHists(AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliAODMCHeader *mcHeader, AliRDHFCutsD0toKpi *cuts, TList *listout, AliAODEvent* aodev);
  ...
  Int_t    fWriteVariableTree;       /// flag to decide whether to write the candidate variables on a tree variables
    //0 don't fill
    //1 fill standard tree (without handler)
    //2 fill tree with topological variables

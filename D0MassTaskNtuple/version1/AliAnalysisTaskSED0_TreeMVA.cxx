...
void AliAnalysisTaskSED0Mass::UserCreateOutputObjects()
{
  ...
  //
    ...
    eles if(fWriteVariableTree==2){
        fTreeHandler = new AliHFCutOptTreeHandler(AliHFCutOptTreeHandler::kD0toKpi,AliHFCutOptTreeHandler::kNsigmaPID);
        if(fSys>0) fTreeHandler->SetUseCentrality();
        fVariablesTree = (TTree*)fTreeHandler->BuildTree("fTreeD0","fTreeD0");
    }
  ...
  if(fWriteVariableTree>0) PostData(7,fVariablesTree);
...

{
  ...
  for (Int_t iD0toKpi = 0; iD0toKpi < nInD0toKpi; iD0toKpi++) {
	...
	DrawDetSignal(d, fDetSignal);
      ...
      FillMassHists(d,mcArray,mcHeader,fCuts,fOutputMass,aod);
...
//____________________________________________________________________________
void AliAnalysisTaskSED0Mass::FillMassHists(AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliAODMCHeader *mcHeader, AliRDHFCutsD0toKpi* cuts, TList *listout, AliAODEvent* aodev){
    //
    ...
    if( fWriteVariableTree==1 && !part->HasBadDaughters()
    ...
    if ((fIsSelectedCandidate==1 || fIsSelectedCandidate==3) && fFillOnlyD0D0bar<2) { //D0
        ...
        if(fReadMC){
            if(labD0>=0) {
                ...
                Int_t origin=AliVertexingHFUtils::CheckOrigin(arrMC,partD0,fUseQuarkTagInKine);
                if(origin==5) {
                    isPrimary=kFALSE; //isPrimary==kFALSE D0fromb isPrimary==kTRUE D0fromc
                    isprompt=kFALSE;
                }
                else if(origin==4){
                    isprompt=kTRUE;
    ...
    if (fIsSelectedCandidate>1 && (fFillOnlyD0D0bar==0 || fFillOnlyD0D0bar==2)) { //D0bar
        ...
        if(fReadMC){
            if(labD0>=0) {
                ...
                //old function    if(CheckOrigin(arrMC,partD0)==5) isPrimary=kFALSE;
                Int_t originn=AliVertexingHFUtils::CheckOrigin(arrMC,partD0,fUseQuarkTagInKine);
                if(originn==5) {
                    isPrimary=kFALSE;
                    isprompt=kFALSE;
                }
    ...
    if(fWriteVariableTree==2){
        fTreeHandler->SetVariables(part,pidHF,masshypo,issignal,isprompt,isrefl);
        if(fSys>0) {
            Float_t evCentr=cuts->GetCentrality((AliAODEvent*)aodev); 
            fTreeHandler->SetCentrality((Char_t)evCentr);
        }
